...
export default function KeepMountedModal({
  ...
  const [nivelOptions, setNivelOptions] = useState(["Enseñanza General Básica Media", "Enseñanza General Básica Superior", "Bachillerato"]);
  ...
  useEffect(() => {
    const usuarioAEditar = usuarios?.filter(
      (usuario) => usuario.submission_uuid === id
    );
    if (usuarioAEditar) {
  ...
  function editar_User(event) {
    ...
    axios
      .patch(`http://ahorasi.dd:8083/webform_rest/profesor/submission/${id}`, {
        webform_id: "profesor",
      ...
      .then((response) => {
        axios
          .get(`http://ahorasi.dd:8083/webform_rest/profesor/submission/${id}`)
          .then((response) => {
