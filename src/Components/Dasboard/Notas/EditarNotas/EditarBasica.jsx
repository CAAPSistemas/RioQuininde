...
export default function KeepMountedModalEditNotasBasica({
  handleOpenNotasB,
  handleCloseNotasB,
  openNotasB,
     id,
  ...
  const isOk = openNotasB;
  useEffect(() => {
    if (isOk) {
      RegistroConductaSemestre1(  id);
      RegistroConductaSemestre2(  id);
      RegistroConductaSemestre3(  id)
    }
  }, [isOk,   id]);
...
console.log(id);
  ...
  function RegistroConductaSemestre1(  id) {
    axios
  ...
  function RegistroConductaSemestre2(  id) {
  ...
  function RegistroConductaSemestre3(  id) {
    axios
      .get("http://ahorasi.dd:8083/webformID?_format=json")
      .then((response) => {
        const nov = response.data;
