...
export default function KeepMountedModal3({
  handleOpened,
  handleCloseadd,
  openedadd,
}) {
...
else
{
  ...
  .then((response) => {
    ...
    return axios.patch(
      `http://ahorasi.dd:8083/webform_rest/estudiante/submission/${re}`,
      { id_user: h }
    );
  })
  .then(() => {
          ...
          <FormControl sx={{ m: 1, width: "25ch" }} variant="outlined">
            ...
            >
              {grupoOptions.map((grupo) => (
                <MenuItem value={grupo} key={grupo}>
                  {grupo}
                </MenuItem>
              ))}
            </Select>
            <FormHelperText id="outlined-nivel-helper">Grupo</FormHelperText>
          </FormControl>
