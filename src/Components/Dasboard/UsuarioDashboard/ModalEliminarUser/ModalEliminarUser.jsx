...
export default function KeepMountedModal1({
  ...
  console.log(id);
  useEffect(() => {
    const usuarioAEliminar = usuarios?.filter(
      (usuario) => usuario.submission_id === id
    );
  ...
  // Eliminar user
  function eliminar_Userr(event) {
    event.preventDefault();
    axios
      .get("http://ahorasi.dd:8083/webformID?_format=json")
      .then((response) => {
        const del = response.data;
        const delJob = del.filter((item) => item.webform_id === "profesor");
        ...
        console.log("cid:" + cid);
        axios
          .get(`http://ahorasi.dd:8083/webform_rest/profesor/submission/${ide}`)
          .then((response) => {
