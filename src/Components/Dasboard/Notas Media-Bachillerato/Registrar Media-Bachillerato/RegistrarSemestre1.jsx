...
export default function TestSemestre1({
  postNotas,
  idM,
  idprofesor,
  grupoE,
  ...
  console.log(postNotas)
  const Semestre1 = "Semestre1";
  const [aeditar, setAeditar] = useState({}); //Semestre 1
  //Semestre1
  useEffect(() => {
    const usuarioAEditar = postNotas?.filter(
      (postNotas) =>
        postNotas.usuario === idM && postNotas.semestre === "Semestre1" && postNotas.asignatura===clase
    );
    if (usuarioAEditar) {
